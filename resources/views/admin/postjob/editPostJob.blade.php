...
<div class="content-wrapper">
  <?php
   /*
    $docsData = $postjobs[0]->document_name;
    // print_r($docsData);
    $data = explode(",", $docsData);
    $document_name = [ 'document_name' => $data];    
    $docCount = count($document_name['document_name']); */
  ?>
  <!-- Page Content header -->
            ...
            <form class="form-horizontal add-employee-form" method="POST" action="{{ route('updatePostjobAdmin') }}" enctype="multipart/form-data" >
              ...
              <div class="box-body">
               ...
               <?php /* <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select class="form-control select2"name="document_name[]" multiple="multiple" data-placeholder="Select a Document" style="width: 100%;" required>
                     ...
                     @for ($i = 0; $i < $docCount; $i++)                      
                      <option value="Adhar Card" {{ (trim($document_name['document_name'][$i]) =='Adhar Card') ? 'selected' : ''  }} >Adhar Card</option>
                      <option value="Pan Card" {{ (trim($document_name['document_name'][$i]) =='Pan Card') ? 'selected':''  }}>Pan Card</option>
                      <option value=">Driving Licence" {{ (trim($document_name['document_name'][$i]) =='Driving Licence') ? 'selected' : ''  }}>Driving Licence</option>
                      <option value="Voter Card" {{ (trim($document_name['document_name'][$i]) =='Voter Card') ? 'selected' : ''  }}>Voter Card</option>
                      <option value="Bank PassBook" {{ (trim($document_name['document_name'][$i]) =='Bank PassBook') ? 'selected' : ''  }}>Bank PassBook</option>
                      <option value="Degree Certificate" {{ (trim($document_name['document_name'][$i]) =='Degree Certificate') ? 'selected' : ''  }}>Degree Certificate</option>
                      @endfor
                    </select>
                  </div>
                </div> */?>
                ...
                <!-- App Deadline -->
                <div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">Application Deadline Date</label>
                  <div class="col-sm-10">
                    <input type="text" name="app_deadline" class="form-control" value="{{ isset($postjobs[0]->app_deadline) ? date( 'm/d/Y', strtotime($postjobs[0]->app_deadline) ) : '' }}" placeholder="Application Deadline Date" id="app_deadline" autocomplete="off" required>
                  </div>
                </div>
                <!-- label MULTI SELECT -->
                <!-- <div class="form-group">
 ...
 -->
              ...
              <!-- Joining port -->
               <div class="form-group">
                <label for="inputPassword3" class="col-sm-2 control-label">Joining Port*:</label>
                <div class="col-sm-10">
                  <input type="text" name="joining_port" class="form-control" placeholder="Enter port of joining" 
                  value="{{ isset($postjobs[0]->joining_port) ? $postjobs[0]->joining_port : ''  }}" required>
                </div>
              </div>
              <!-- Country -->
                <!-- <div class="form-group">
...
//         // document.getElementById("whereToPrint").innerHTML = JSON.stringify(obj);
        ...
        if (wage != "") 
        {
            ...
            // $.each(aVal, function(i, value) {   
              ...
              result +="<thead><tr><th>Rank</th><th>Contract Duration (in months)</th><th>Wages (In Dollar)</th></tr></thead>";
          $.each(obj,function(i,ob){
            ...
            result +="<td><input type='number' step='0.01' name='contract_duration[]' 0.5='' class='form-control col-sm-3' placeholder='Contract Duration (in months)' value='"+ ob.contract_duration +"' required></td>";
            ...
            result +="<td><input type='number' class='BoxValClass form-control col-sm-3' name='wage[]' id='BoxVal' placeholder='Enter Wages' value='"+ob.wages+"'' ></td></tr>";
    ...
    //Add button
      ...
      $("#addButton").click(function () {
       ...
       //alert('Count is :'+count)
        ...
        newTextBoxDiv.after().html('<td><select onchange="getval(this);" class="form-control rank" id="rank[' + counter + ']" name="rank[' + counter + ']">'+htmloptions+'</select></td><td><input type="number" step=0.01 name="contract_duration[]"  Step 0.5 class="form-control" placeholder="Contract Duration (in months)" required /></td><td><input type="number" class="BoxValClass form-control" name="wage[]" placeholder="Enter Wages"></td>');
